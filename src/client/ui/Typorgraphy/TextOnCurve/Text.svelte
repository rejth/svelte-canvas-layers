<script lang="ts">
  ...
  import { ArcLengthMap } from '../geometry/ArcLengthMap';

  /**
   * Plots the text string along the curve.
   * If spread is given, it should be a scale factor for the intra-letter spacing (1 means normal spacing).
   * Shift is the starting value of "t" where the middle of the string is to be written (0.5 by default).
   **/

  export let text: string;
  export let font: string;
  export let curve: Bezier;
  export let spread = 1;
  ...
  $: render = ({ ctx }: RenderProps) => {
    ...
    const map = new ArcLengthMap(curve);
    ...
    for (let i = 1; i <= text.length; i++) {
      ...
      const tan = curve.tangent(baset);
      const pos = curve.point(baset).sub(tan.scale((wordWidth - lastWidth) / 2));
